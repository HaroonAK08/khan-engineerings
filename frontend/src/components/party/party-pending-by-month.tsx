...
export function PartyPendingByMonth({
  ...
  const leftoverMonths = useMemo(() => {
    ...
    return snapshot.months.filter((m) => {
      if (m.remaining <= 0.001) return false;
      if (!q) return true;
    ...
    <div className="flex flex-col gap-4">
      {hasRange ? (
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {stats.map((s) => (
            <div key={s.key} className="rounded-lg border bg-muted/30 px-3 py-3">
                ...
                {leftoverMonths.map((m) => {
                  ...
                  return (
                    <TableRow key={m.month} className={inView ? "bg-primary/5" : undefined}>
                      ...
                      <TableCell className="font-data text-right text-sm font-semibold text-amber-700 dark:text-amber-400">
